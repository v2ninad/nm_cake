 ...
 */
	...
	/*
	 * Records whether session has been updated. If not, there is no need to do a final database update when closing the session.
	 */
	static $hasBeenUpdated = false;
	...
	function _checkValid() {
		if ($this->read('Config')) {
			if ((Configure::read('Session.checkAgent') === false || $this->_userAgent == $this->read('Config.userAgent')) && $this->time <= $this->read('Config.time')) {
				...
				//only re-write the session timeout if less than "inactive minutes" setting
				$timeRemaining = $this->read('Config.time') - $this->time;
				if($timeRemaining < (Security::inactiveMins() * 60) ){
 ...
 */
	function __write($key, $value) {
		...
		if(ConnectionManager::$isDestroyed && self::$hasBeenUpdated == false){
			//Not updating database because we are destroying the session and session has not been updated!
			return false;
		}
